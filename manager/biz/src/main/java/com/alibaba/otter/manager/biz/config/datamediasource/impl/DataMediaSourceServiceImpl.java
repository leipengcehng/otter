...
public class DataMediaSourceServiceImpl implements DataMediaSourceService {
    ...
    /**
     * 类型：数据库类型 Mysql和Oracle、Postgresql 用于DO对象转化为Model对象
     * 
    ...
    private DataMediaSource doToModel(DataMediaSourceDO dataMediaSourceDo) {
        ...
        try {
            if (dataMediaSourceDo.getType().isMysql() || dataMediaSourceDo.getType().isOracle() || dataMediaSourceDo.getType().isPostgresql()) {
