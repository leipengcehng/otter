...
public class DdlUtils {
    ...
    /**
     * 对MySQL的ddl语句进行转换，转换为PostgreSql的ddl语句
     * @param sql
     * @param data
     * @return
     */
    public static String convertToPostgreSql(String sql, EventData data) {
        String newSql = "";
        String originalSql = data.getSql();
        originalSql = originalSql.toLowerCase();
        originalSql = originalSql.replaceAll("`", "\"");
        if (data.getEventType().isCreate() || data.getEventType().isAlter()) {
            if (originalSql.contains("AUTO_INCREMENT".toLowerCase())) {
                originalSql = originalSql.replaceAll("AUTO_INCREMENT".toLowerCase(), "");
            }
            if (originalSql.contains("CHARACTER SET ucs2".toLowerCase())) {
                originalSql = originalSql.replaceAll("CHARACTER SET ucs2".toLowerCase(), "");
            }
            if (originalSql.contains("PRIMARY KEY".toLowerCase())) {
                String pkStr = originalSql.substring(originalSql.indexOf("PRIMARY KEY".toLowerCase()));
                String replaceStr = "CONSTRAINT " + data.getTableName() + "_pkey PRIMARY KEY (\"id\"));";
                originalSql = originalSql.replace(pkStr, replaceStr);
            }
            if (data.getEventType().isAlter() && originalSql.contains("change")){
                //mysql修改列名同步postgresql
                String oldColumnName = "";
                String newColumnName = "";
                String newColumnType = "";
                String[] alterSqlArray = originalSql.split("\\s+");
                for (int i = 0; i < alterSqlArray.length; i++) {
                    if ("change".equals(alterSqlArray[i])) {
                        oldColumnName = alterSqlArray[i+1];
                        newColumnName = alterSqlArray[i+2];
                        newColumnType = alterSqlArray[i+3];
                    }
                }
                String beginSql = originalSql.substring(0, originalSql.indexOf("change"));
                originalSql = beginSql + " rename " + oldColumnName + " to " + newColumnName + ";";
                String alterColumnTypeSql = beginSql + " alter " + newColumnName + " type " + newColumnType;
                originalSql += alterColumnTypeSql;
            }
            if (data.getEventType().isAlter() && originalSql.contains("modify")) {
                String columnName = "";
                String columnType = "";
                String[] alterSqlArray = originalSql.split("\\s+");
                for (int i = 0; i < alterSqlArray.length; i++) {
                    if ("modify".equals(alterSqlArray[i])) {
                        columnName = alterSqlArray[i+1];
                        columnType = alterSqlArray[i+2];
                    }
                }
                String beginSql = originalSql.substring(0, originalSql.indexOf("modify"));
                originalSql = beginSql + " alter " + columnName + " type " + columnType;
            }
            while (originalSql.contains("COMMENT '".toLowerCase())) {
                int indexComment = originalSql.indexOf("COMMENT '".toLowerCase());
                int nextComma = originalSql.indexOf(",", indexComment);
                originalSql = originalSql.replace(originalSql.substring(indexComment, nextComma), "");
            }
            String[] sqlArray = originalSql.split("\\s+");
            for (int i = 0; i < sqlArray.length; i++) {
                if (sqlArray[i].startsWith("bigint")) {
                    sqlArray[i] = "int8";
                } else if (sqlArray[i].startsWith("int")) {
                    sqlArray[i] = "int4";
                } else if (sqlArray[i].startsWith("double")) {
                    sqlArray[i] = "float8";
                } else if (sqlArray[i].startsWith("tinyint")) {
                    sqlArray[i] = "int2";
                } else if (sqlArray[i].startsWith("smallint")) {
                    sqlArray[i] = "int2";
                }
                newSql += sqlArray[i] + " ";
            }
