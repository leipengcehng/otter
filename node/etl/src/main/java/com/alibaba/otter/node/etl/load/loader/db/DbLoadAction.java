...
import com.alibaba.otter.node.etl.common.db.dialect.gp.PostgreSQLDialect;
            ...
            try {
                Boolean result = dbDialect.getJdbcTemplate().execute(new StatementCallback<Boolean>() {
                    ...
                    public Boolean doInStatement(Statement stmt) throws SQLException, DataAccessException {
                        ...
                        } else if (dbDialect instanceof PostgreSQLDialect && StringUtils.isNotEmpty(data.getDdlSchemaName())) {
                            if (data.getEventType().isCreate()) {
                                result &= stmt.execute(data.getSql());
//                            result &= stmt.execute("/c " + data.getDdlSchemaName());
                                if (data.getSql().contains("tenant_id") && data.getEventType().isCreate()) {
                                    String indexSql = "CREATE INDEX \"" + data.getTableName() +"_tenant_id_idx\" ON \"public\".\""+ data.getTableName() +"\" USING btree (\n" +
                                            "  \"tenant_id\" \"pg_catalog\".\"int8_ops\"\n" +
                                            ");";
                                    result &= stmt.execute(indexSql);
                                }
                            } else if (data.getEventType().isAlter()) {
                                String[] sqlArray = data.getSql().trim().split(";");
                                if (sqlArray.length > 1) {
                                    result &= stmt.execute(sqlArray[0]);
                                    result &= stmt.execute(sqlArray[1]);
                                } else {
                                    result &= stmt.execute(data.getSql());
                                }
                            } else {
        ...
        private void doPreparedStatement(PreparedStatement ps, DbDialect dbDialect, LobCreator lobCreator,
            ...
            for (int i = 0; i < columns.size(); i++) {
                ...
                if (dbDialect instanceof MysqlDialect
                    ...
                    if (dbDialect instanceof PostgreSQLDialect && sqlType == Types.VARCHAR && StringUtils.isNotEmpty(column.getColumnValue())) {
                        param = SqlUtils.stringToSqlValue(column.getColumnValue().replaceAll("\u0000", ""),
                                sqlType,
                                isRequired,
                ...
                try {
                    switch (sqlType) {
                        ...
                        case Types.DATE:
                            ...
                            if (dbDialect instanceof MysqlDialect || dbDialect instanceof PostgreSQLDialect) {
                                // 解决mysql的0000-00-00 00:00:00问题，直接依赖mysql
                                // driver进行处理，如果转化为Timestamp会出错
